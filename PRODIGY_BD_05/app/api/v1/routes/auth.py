...
def register(payload: LoginRequest, role: UserRole = UserRole.USER, service : AuthService = Depends(get_auth_service_v1), db: Session = Depends(get_db)):
...
def login(payload: LoginRequest, service : AuthService = Depends(get_auth_service_v1), db: Session = Depends(get_db)):
